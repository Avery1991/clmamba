...
class VSSM(nn.Module):
    def __init__(
        ...
        self.patch_embed = _make_patch_embed(in_chans, dims[0], patch_size, patch_norm, norm_layer,
                                             channel_first=self.channel_first)
        self.patch_embed1 = _make_patch_embed(1, dims[0], patch_size, patch_norm, norm_layer,
                                              channel_first=self.channel_first)
...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", **kwargs):
        ...
        # self.clf = nn.ModuleList([CLF(channels=96), CLF(channels=192), CLF(channels=384), CLF(channels=768)])
